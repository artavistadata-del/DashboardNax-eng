...
import { MapPin, Plane, Ship, Truck, CheckCircle2, Circle, Clock, AlertTriangle, RefreshCw } from 'lucide-react';

const trackingData = [
  {
    awb: 'NAX-20240501',
    client: 'Tsukiji Fish Market',
    product: 'Bluefin Tuna — 420 kg',
    type: 'Air Perishable',
    typeColor: '#1e3a8a',
    origin: 'Tokyo (NRT)',
    dest: 'Los Angeles (LAX)',
    eta: '2026-05-16 08:30',
    temp: '-2°C',
    currentLeg: 2,
    milestones: [
      { label: 'Booking Confirmed', loc: 'Tokyo, JP', time: '2026-05-14 09:00', done: true, alert: false },
      { label: 'Cargo Collected & Re-iced', loc: 'NRT Cold Storage', time: '2026-05-14 14:30', done: true, alert: false },
      { label: 'Departed — NRT', loc: 'Narita Intl Airport', time: '2026-05-14 22:00', done: true, alert: false },
      { label: 'In Flight', loc: 'Pacific Airspace', time: '2026-05-15 02:00', done: false, alert: false },
      { label: 'Arrived — LAX', loc: 'Los Angeles Intl', time: '2026-05-15 18:00', done: false, alert: false },
      { label: 'Customs Clearance', loc: 'LAX CBP', time: '2026-05-15 20:00', done: false, alert: false },
      { label: 'Delivered to Client', loc: 'Los Angeles, CA', time: '2026-05-16 08:30', done: false, alert: false },
    ],
  },
  {
    awb: 'NAX-20240503',
    client: 'Fresh Mart LA',
    product: 'Fresh Produce — 3,800 kg',
    type: 'Ocean Freight',
    typeColor: '#f59e0b',
    origin: 'Mexico City (MEX)',
    dest: 'Los Angeles (LAX)',
    eta: '2026-05-18 15:00',
    temp: '4°C',
    currentLeg: 3,
    milestones: [
      { label: 'Booking Confirmed', loc: 'Mexico City, MX', time: '2026-05-10 10:00', done: true, alert: false },
      { label: 'Cargo Loaded at Port', loc: 'Manzanillo Port', time: '2026-05-12 06:00', done: true, alert: false },
      { label: 'Vessel Departed', loc: 'Pacific Ocean', time: '2026-05-12 18:00', done: true, alert: false },
      { label: 'FDA Inspection Hold', loc: 'Long Beach Port', time: '2026-05-15 10:00', done: false, alert: true },
      { label: 'Customs Release', loc: 'LA Customs', time: '2026-05-17 12:00', done: false, alert: false },
      { label: 'Distribution', loc: 'NAX LA Warehouse', time: '2026-05-18 09:00', done: false, alert: false },
      { label: 'Delivered', loc: 'Los Angeles, CA', time: '2026-05-18 15:00', done: false, alert: false },
    ],
  },
  {
    awb: 'NAX-20240504',
    client: 'Blue Ocean Seafood',
    product: 'Sea Urchin — 650 kg',
    type: 'Refrigerated Truck',
    typeColor: '#10b981',
    origin: 'Los Angeles (LAX)',
    dest: 'Newark (NWK)',
    eta: '2026-05-17 09:00',
    temp: '2°C',
    currentLeg: 1,
    milestones: [
      { label: 'Pickup Scheduled', loc: 'LAX Cold Storage', time: '2026-05-15 08:00', done: true, alert: false },
...
const typeIcon: Record<string, JSX.Element> = {
  'Air Perishable': <Plane size={14}/>,
  'Ocean Freight': <Ship size={14}/>,
  'Refrigerated Truck': <Truck size={14}/>,
};
      ...
      <div className="space-y-3">
        ...
        {trackingData.map(s => (
          <button key={s.awb} onClick={() => setSelected(s)}
            ...
            <p className="text-xs font-medium mb-3" style={{ color: '#94a3b8' }}>{s.product}</p>
            ...
            <p className="text-xs font-medium mt-1" style={{ color: '#94a3b8' }}>
              Step {s.currentLeg}/{s.milestones.length}
            </p>
          </button>
        ))}
      </div>
      ...
      <div className="lg:col-span-2 space-y-5">
        ...
        <div className="glass-card p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-black text-base" style={{ color: '#0f172a' }}>Tracking Timeline</h3>
                ...
                <div key={i} className="flex gap-4">
                  ...
                  <div className={`pb-6 flex-1 ${isLast ? '' : ''}`}>
                    <div className="flex items-start justify-between">
                      <div>
                        <p className={`text-sm font-black ${m.done ? '' : m.alert ? '' : isCurrent ? '' : 'opacity-50'}`}
                          style={{ color: m.done ? '#0f172a' : m.alert ? '#ef4444' : isCurrent ? '#f59e0b' : '#94a3b8' }}>
                          {m.label}
                          {m.alert && <span className="ml-2 text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(239,68,68,0.1)', color: '#ef4444' }}>⚠ Hold</span>}
                      ...
                      <p className="text-xs font-semibold text-right" style={{ color: '#94a3b8' }}>{m.time}</p>
                    </div>
                  </div>
                </div>
              );

...
import { Search, Bell, Calendar, Download, ChevronDown } from 'lucide-react';
...
export default function Header({ activeMenu, unreadAlerts = 3 }: HeaderProps) {
  const page = pageTitles[activeMenu] ?? { title: 'Dashboard', sub: '' };

  return (
    <header className="flex-shrink-0 px-8 py-4 flex items-center justify-between sticky top-0 z-30"
      style={{
      ...
      {/* Left — Page title */}
      <div>
        <h1 className="text-lg font-black" style={{ color: '#0f2557' }}>{page.title}</h1>
        <p className="text-xs font-medium" style={{ color: '#94a3b8' }}>{page.sub}</p>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative hidden md:block">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 text-sm font-medium rounded-xl outline-none transition-all w-56"
            style={{
        ...
        {/* Export */}
        <button className="hidden lg:flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-white transition-all"
          style={{
        ...
        {/* Avatar */}
        <div className="flex items-center gap-2.5 pl-3 border-l" style={{ borderColor: 'rgba(0,0,0,0.08)' }}>
